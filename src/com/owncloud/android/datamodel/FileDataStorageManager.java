...
public class FileDataStorageManager {
    ...
    public Vector<OCFile> getFolderImages(OCFile folder) {
        ...
        if (folder != null) {
            // TODO better implementation, filtering in the access to database
            // (if possible) instead of here
    ...
    public boolean saveFile(OCFile file) {
        ...
        if (sameRemotePath || fileExists(file.getFileId())) { // for renamed
                                                              // files; no more
                                                              // delete and
                                                              // create
            ...
            if (getContentResolver() != null) {
                ...
                try {
                    getContentProviderClient().update(ProviderTableMeta.CONTENT_URI, cv, ProviderTableMeta._ID + "=?",
                            new String[] { String.valueOf(file.getFileId()) });
     ...
     * 
     * CALLER IS THE RESPONSIBLE FOR GRANTING RIGHT UPDATE OF INFORMATION, NOT
     * THIS METHOD. HERE ONLY DATA CONSISTENCY SHOULD BE GRANTED
     * 
     * @param folder
     * @param files
    ...
    public void saveFolder(OCFile folder, Collection<OCFile> updatedFiles, Collection<OCFile> filesToRemove) {
        ...
        // prepare operations to insert or update files to save in the given
        // folder
        for (OCFile file : updatedFiles) {
            ...
            if (existsByPath || fileExists(file.getFileId())) {
                ...
                // adding a new file
                operations
                        .add(ContentProviderOperation.newInsert(ProviderTableMeta.CONTENT_URI).withValues(cv).build());
            }
        }
        ...
        for (OCFile file : filesToRemove) {
            if (file.getParentId() == folder.getFileId()) {
                ...
                // Uri.withAppendedPath(ProviderTableMeta.CONTENT_URI_FILE, "" +
                // file.getFileId());
                if (file.isFolder()) {
                    operations.add(ContentProviderOperation
                            .newDelete(ContentUris.withAppendedId(ProviderTableMeta.CONTENT_URI_DIR, file.getFileId()))
                            ...
                            .add(ContentProviderOperation
                                    .newDelete(
                                            ContentUris.withAppendedId(ProviderTableMeta.CONTENT_URI_FILE,
                                                    file.getFileId())).withSelection(where, whereArgs).build());
                    if (file.isDown()) {
                        new File(file.getStoragePath()).delete();
                        // TODO move the deletion of local contents after
                        // success of deletions
                    }
                }
            }
        }
        ...
        cv.put(ProviderTableMeta.FILE_CONTENT_LENGTH, 0); // FileContentProvider
                                                          // calculates the
                                                          // right size
    ...
    private void updateFolderSize(long id) {
        if (id > FileDataStorageManager.ROOT_PARENT_ID) {
            ...
            if (getContentResolver() != null) {
                getContentResolver().update(ProviderTableMeta.CONTENT_URI_DIR, new ContentValues(), // won't
                                                                                                    // be
                                                                                                    // used,
                                                                                                    // but
                                                                                                    // cannot
                                                                                                    ...
                                                                                                    // in
                                                                                                    // KLP
                        ProviderTableMeta._ID + "=?", new String[] { String.valueOf(id) });
            } else {
                try {
                    getContentProviderClient().update(ProviderTableMeta.CONTENT_URI_DIR, new ContentValues(), // won't
                                                                                                              // be
                                                                                                              // used,
                                                                                                              // but
                                                                                                              // cannot
            ...
            if (file.isFolder()) {
                ...
                if (removeLocalCopy && file.isDown() && file.getStoragePath() != null) {
                    ...
                    if (!removeDBData && success) {
                        // maybe unnecessary, but should be checked TODO remove
                        // if unnecessary
    ...
    public void moveFolder(OCFile folder, String newPath) {
        ...
        if (folder != null && folder.isFolder() && folder.fileExists()
                && !OCFile.ROOT_PATH.equals(folder.getFileName())) {
            // / 1. get all the descendants of 'dir' in a single QUERY
            // (including 'dir')
            Cursor c = null;
            if (getContentProviderClient() != null) {
                try {
                    c = getContentProviderClient()
                            .query(ProviderTableMeta.CONTENT_URI,
                                    null,
                                    ProviderTableMeta.FILE_ACCOUNT_OWNER + "=? AND " + ProviderTableMeta.FILE_PATH
                                            + " LIKE ? ", new String[] { mAccount.name, folder.getRemotePath() + "%" },
            ...
            // / 2. prepare a batch of update operations to change all the
            // descendants
            ...
            if (c.moveToFirst()) {
                do {
                    ContentValues cv = new ContentValues(); // don't take the
                                                            // constructor out
                                                            // of the loop and
                                                            // clear the object
                    ...
                    if (child.getStoragePath() != null && child.getStoragePath().startsWith(defaultSavePath)) {
                        cv.put(ProviderTableMeta.FILE_STORAGE_PATH, defaultSavePath + newPath
                    ...
                    operations.add(ContentProviderOperation
                            ...
                            .withSelection(ProviderTableMeta._ID + "=?",
                                    new String[] { String.valueOf(child.getFileId()) }).build());
                } while (c.moveToNext());
            }
            c.close();
        ...
        if (c != null) {
            ...
            if (!file.isFolder()) {
                ...
                if (file.getStoragePath() == null) {
                    // try to find existing file and bind it with current
                    // account; - with the current update of
                    // SynchronizeFolderOperation, this won't be necessary
                    // anymore after a full synchronization of the account
