...
public class FileObserverService extends Service {
    ...
    public void onDestroy() {
        ...
        mObserversMap = null; // TODO study carefully the life cycle of Services
                              // to grant the best possible observance
    ...
    private void initializeObservedList() {
        mObserversMap.clear();
        Cursor c = getContentResolver().query(ProviderTableMeta.CONTENT_URI, null,
        ...
        do {
            ...
            if (path == null || path.length() <= 0)
                continue;
            OwnCloudFileObserver observer = new OwnCloudFileObserver(path, account, getApplicationContext());
     ...
     * file.
     * 
     * TODO We are ignoring that, currently, a local file can be linked to
     * different files in ownCloud if it's uploaded several times. That's
     * something pending to update: we will avoid that the same local file is
     * linked to different remote files.
     * 
    ...
    private void addObservedFile(OCFile file, Account account) {
        ...
        if (localPath == null || localPath.length() <= 0) { // file downloading
                                                            // / to be download
                                                            // for the first
                                                            // time
        ...
        if (observer == null) {
            ...
            if (file.isDown()) {
              ...
              // down; mDownloadReceiver will get noticed when the download of
              // the file finishes
        }
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (observer != null) {
                if (intent.getAction().equals(FileDownloader.getDownloadFinishMessage())
                        && new File(downloadPath).exists()) { // the download
                                                              // could be
                                                              // successful.
                                                              // not; in both
                                                              // cases, the file
