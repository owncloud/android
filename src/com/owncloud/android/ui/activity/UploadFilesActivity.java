...
/**
 * Displays local files and let the user choose what of them wants to upload to
 * the current ownCloud account
 * 
...
public class UploadFilesActivity extends FileActivity implements LocalFileListFragment.ContainerActivity,
        OnNavigationListener, OnClickListener, ConfirmationDialogFragmentListener {
    ...
    public static final String EXTRA_CHOSEN_FILES = UploadFilesActivity.class.getCanonicalName()
            + ".EXTRA_CHOSEN_FILES";

    public static final int RESULT_OK_AND_MOVE = RESULT_FIRST_USER;

    private static final String KEY_DIRECTORY_PATH = UploadFilesActivity.class.getCanonicalName()
            + ".KEY_DIRECTORY_PATH";
     ...
     * @throws IllegalArgumentException If the {@link File#isDirectory()}
     *             returns false.
     */
    ...
    /**
     * Performs corresponding action when user presses 'Cancel' or 'Upload'
     * button
     * 
     * TODO Make here the real request to the Upload service ; will require to
     * receive the account and target folder where the upload must be done in
     * the received intent.
     */
    @Override
    ...
    /**
     * Asynchronous task checking if there is space enough to copy all the files
     * chosen to upload into the ownCloud local folder.
     * 
    ...
    private class CheckAvailableSpaceTask extends AsyncTask<Void, Void, Boolean> {
         ...
         * 
         * If there is not space enough. shows a new dialog to query the user if
         * wants to move the files instead of copy them.
         * 
         * @param result 'True' when there is space enough to copy all the
         *            selected files.
         */
        @Override
        protected void onPostExecute(Boolean result) {
            ...
            if (result) {
                ...
                // show a dialog to query the user if wants to move the selected
                // files to the ownCloud folder instead of copying
    ...
    public void onConfirmation(String callerTag) {
        ...
        if (callerTag.equals(QUERY_TO_MOVE_DIALOG_TAG)) {
            // return the list of selected files to the caller activity
            // (success), signaling that they should be moved to the ownCloud
            // folder, instead of copied
            Intent data = new Intent();
            data.putExtra(EXTRA_CHOSEN_FILES, mFileListFragment.getCheckedFilePaths());
