...
public class FileDetailFragment extends FileFragment implements OnClickListener,
     ...
     * 
     * When 'fileToDetail' or 'ocAccount' are null, creates a dummy layout (to
     * use when a file wasn't tapped before).
     * 
    ...
    private void toggleKeepInSync() {
        ...
        intent.putExtra(FileObserverService.KEY_FILE_CMD, (cb.isChecked() ? FileObserverService.CMD_ADD_OBSERVED_FILE
    ...
    private void removeFile() {
        ...
        ConfirmationDialogFragment confDialog = ConfirmationDialogFragment.newInstance(
                R.string.confirmation_remove_alert, new String[] { file.getFileName() },
                file.isDown() ? R.string.confirmation_remove_remote_and_local : R.string.confirmation_remove_remote,
                file.isDown() ? R.string.confirmation_remove_local : -1, R.string.common_cancel);
    ...
    public void onConfirmation(String callerTag) {
        ...
        if (callerTag.equals(FTAG_CONFIRMATION)) {
            if (mStorageManager.getFileById(file.getFileId()) != null) {
                mLastRemoteOperation = new RemoveFileOperation(file, true, mStorageManager);
                mLastRemoteOperation.execute(mAccount, getSherlockActivity(), this, mHandler, getSherlockActivity());
    ...
    public void onNeutral(String callerTag) {
        OCFile file = getFile();
        mStorageManager.removeFile(file, false, true); // TODO perform in
                                                       // background task / new
                                                       // thread
    ...
    /**
     * Check if the fragment was created with an empty layout. An empty fragment
     * can't show file details, must be replaced.
     * 
    ...
    public void updateFileDetails(OCFile file, Account ocAccount) {
        setFile(file);
        if (ocAccount != null && (mStorageManager == null || (mAccount != null && !mAccount.equals(ocAccount)))) {
            mStorageManager = new FileDataStorageManager(ocAccount, getActivity().getApplicationContext()
                    .getContentResolver());
        }
        mAccount = ocAccount;
        updateFileDetails(false, false);
     ...
     * 
     * TODO Remove parameter when the transferring state of files is kept in
     * database.
     * 
     * TODO REFACTORING! this method called 5 times before every time the
     * fragment is shown!
     * 
     * @param transferring Flag signaling if the file should be considered as
     *            downloading or uploading, although
     *            {@link FileDownloaderBinder#isDownloading(Account, OCFile)}
    ...
    public void updateFileDetails(boolean transferring, boolean refresh) {
        ...
        if (readyToShow()) {
            ...
            FileDownloaderBinder downloaderBinder = mContainerActivity.getFileDownloaderBinder();
            FileUploaderBinder uploaderBinder = mContainerActivity.getFileUploaderBinder();
            if (transferring || (downloaderBinder != null && downloaderBinder.isDownloading(mAccount, file))
                ...
                // TODO load default preview image; when the local file is
                // removed, the preview remains there
                setButtonsForRemote();
            }
        }
        getView().invalidate();
     ...
     * the time that the file was created. There is a chance that this will be
     * fixed in future versions. Use this method to check if this version of
     * ownCloud has this fix.
     * 
     * @return True, if ownCloud the ownCloud version is supporting creation
     *         time
     */
    private boolean ocVersionSupportsTimeCreated() {
        /*
         * if(mAccount != null){ AccountManager accManager = (AccountManager)
         ...
         * if(ocVersion.compareTo(new OwnCloudVersion(0x030000)) < 0) { return
         * true; } }
         */
        return false;
    }
     ...
     * 
     * Being notified about the finish of an upload is necessary for the next
     * sequence: 1. Upload a big file. 2. Force a synchronization; if it
     * finished before the upload, the file in transfer will be included in the
     * local database and in the file list of its containing folder; the the
     * server includes it in the PROPFIND requests although it's not fully
     * upload. 3. Click the file in the list to see its details. 4. Wait for the
     * upload finishes; at this moment, the details view must be refreshed to
    ...
    private class UploadFinishReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (!isEmpty() && accountName.equals(mAccount.name)) {
                ...
                if (getFile().getRemotePath().equals(uploadRemotePath) || renamedInUpload) {
                    if (uploadWasFine) {
                    ...
                    getSherlockActivity().removeStickyBroadcast(intent); // not
                                                                         // the
                                                                         // best
                                                                         // place
                                                                         // to
                    ...
                    updateFileDetails(false, false); // it updates the buttons;
                                                     // must be called although
                                                     // !uploadWasFine;
                                                     // interrupted uploads
                                                     // still leave an
                                                     // incomplete file in the
                                                     // server
    ...
    public void onDismiss(EditNameDialog dialog) {
        if (dialog.getResult()) {
            ...
            mLastRemoteOperation = new RenameFileOperation(getFile(), mAccount, newFilename,
    ...
    private void onSynchronizeFileOperationFinish(SynchronizeFileOperation operation, RemoteOperationResult result) {
        ...
        if (!result.isSuccess()) {
            ...
            if (operation.transferWasRequested()) {
                setButtonsForTransferring();
                mContainerActivity.onFileStateChanged(); // this is not working;
                                                         // FileDownloader won't
                                                         // do NOTHING at all
                                                         // until this method
                                                         // finishes, so
    ...
    public void listenForTransferProgress() {
        if (mProgressListener != null) {
            if (mContainerActivity.getFileDownloaderBinder() != null) {
                mContainerActivity.getFileDownloaderBinder().addDatatransferProgressListener(mProgressListener,
                        mAccount, getFile());
            }
            if (mContainerActivity.getFileUploaderBinder() != null) {
                mContainerActivity.getFileUploaderBinder().addDatatransferProgressListener(mProgressListener, mAccount,
                        getFile());
            }
        }
    }
    ...
    public void leaveTransferProgress() {
        if (mProgressListener != null) {
            if (mContainerActivity.getFileDownloaderBinder() != null) {
                mContainerActivity.getFileDownloaderBinder().removeDatatransferProgressListener(mProgressListener,
                        mAccount, getFile());
            }
            if (mContainerActivity.getFileUploaderBinder() != null) {
                mContainerActivity.getFileUploaderBinder().removeDatatransferProgressListener(mProgressListener,
                        mAccount, getFile());
            }
        }
    }
    ...
    /**
     * Helper class responsible for updating the progress bar shown for file
