 ...
 * 
 * Currently, it always plays in landscape mode, full screen. When the playback
 * ends, the activity is finished.
 * 
 * @author David A. Velasco
 */
public class PreviewVideoActivity extends FileActivity implements OnCompletionListener, OnPreparedListener,
        OnErrorListener {

    /**
     * Key to receive a flag signaling if the video should be started
     * immediately
     */
    ...
    private int mSavedPlaybackPosition; // in the unit time handled by
                                        // MediaPlayer.getCurrentPosition()
    private boolean mAutoplay; // when 'true', the playback starts immediately
                               // with the activity
    private VideoView mVideoPlayer; // view to play the file; both performs and
                                    // show the playback
    private MediaController mMediaController; // panel control used by the user
                                              // to control the playback

    /**
     * Called when the activity is first created.
     * 
     * Searches for an {@link OCFile} and ownCloud {@link Account} holding it in
     * the starting {@link Intent}.
     * 
     * The {@link Account} is unnecessary if the file is downloaded; else, the
     * {@link Account} is used to try to stream the remote file - TODO get the
     * streaming works
     * 
     * {@inheritDoc}
     */
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        // keep the screen on while the playback is performed (prevents screen
        // off by battery save)
