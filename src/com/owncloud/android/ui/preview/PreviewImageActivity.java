...
/**
 * Holds a swiping galley where image files contained in an ownCloud directory
 * are shown
 * 
...
public class PreviewImageActivity extends FileActivity implements FileFragment.ContainerActivity,
    ...
    private void initViewPager() {
        ...
        // OCFile parentFolder =
        // mStorageManager.getFileById(getFile().getParentId());
        ...
        if (position == 0 && !getFile().isDown()) {
            // this is necessary because mViewPager.setCurrentItem(0) just after
            // setting the adapter does not result in a call to
            // #onPageSelected(0)
     ...
     * current page. when it is fully stopped/idle.
     * 
     * @param State The new scroll state (SCROLL_STATE_IDLE, _DRAGGING,
     *            _SETTLING
     */
    @Override
     ...
     * is relevant for the folder displayed in the gallery.
     */
    private class DownloadFinishReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (getAccount().name.equals(accountName) && downloadedRemotePath != null) {
                ...
                // boolean isOffscreen = Math.abs((mViewPager.getCurrentItem() -
                // position)) <= mViewPager.getOffscreenPageLimit();
                ...
                if (position >= 0 && intent.getAction().equals(FileDownloader.getDownloadFinishMessage())) {
                    ...
                    mPreviewImagePagerAdapter.notifyDataSetChanged(); // will
                                                                      // trigger
                                                                      // the
                                                                      // creation
                                                                      // of new
    ...
    protected void onAccountSet(boolean stateWasRecovered) {
        if (getAccount() != null) {
            ...
            if (file != null) {
                // / Refresh the activity according to the Account and OCFile
                // set
                setFile(file); // reset after getting it fresh from
                               // mStorageManager
