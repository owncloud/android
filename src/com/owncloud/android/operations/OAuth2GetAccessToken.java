...
public class OAuth2GetAccessToken extends RemoteOperation {
    ...
    public OAuth2GetAccessToken(String clientId, String redirectUri, String grantType,
    ...
    protected RemoteOperationResult run(WebdavClient client) {
        ...
        try {
            ...
            if (mOAuth2ParsedAuthorizationResponse.keySet().contains(OAuth2Constants.KEY_ERROR)) {
                if (OAuth2Constants.VALUE_ERROR_ACCESS_DENIED.equals(mOAuth2ParsedAuthorizationResponse
                        .get(OAuth2Constants.KEY_ERROR))) {
            ...
            if (result == null) {
                ...
                // nameValuePairs[4] = new
                // NameValuePair(OAuth2Constants.KEY_SCOPE,
                // mOAuth2ParsedAuthorizationResponse.get(OAuth2Constants.KEY_SCOPE));
            ...
            if (postMethod != null)
                postMethod.releaseConnection(); // let the connection available
                                                // for other methods

            if (result.isSuccess()) {
                Log_OC.i(TAG, "OAuth2 TOKEN REQUEST with auth code " + mOAuth2ParsedAuthorizationResponse.get("code")
                        + " to " + client.getBaseUri() + ": " + result.getLogMessage());

            } else if (result.getException() != null) {
                Log_OC.e(TAG, "OAuth2 TOKEN REQUEST with auth code " + mOAuth2ParsedAuthorizationResponse.get("code")
                        + " to " + client.getBaseUri() + ": " + result.getLogMessage(), result.getException());

            } else if (result.getCode() == ResultCode.OAUTH2_ERROR) {
                Log_OC.e(
                        TAG,
                        "OAuth2 TOKEN REQUEST with auth code " + mOAuth2ParsedAuthorizationResponse.get("code")
                                + " to " + client.getBaseUri() + ": "
    ...
    private void parseAccessTokenResult(JSONObject tokenJson) throws JSONException {
        ...
        if (tokenJson.has(OAuth2Constants.KEY_ACCESS_TOKEN)) {
            mResultTokenMap
                    .put(OAuth2Constants.KEY_ACCESS_TOKEN, tokenJson.getString(OAuth2Constants.KEY_ACCESS_TOKEN));
        }
        ...
        if (tokenJson.has(OAuth2Constants.KEY_ERROR_DESCRIPTION)) {
            mResultTokenMap.put(OAuth2Constants.KEY_ERROR_DESCRIPTION,
                    tokenJson.getString(OAuth2Constants.KEY_ERROR_DESCRIPTION));
        }
