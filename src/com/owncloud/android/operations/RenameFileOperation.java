...
public class RenameFileOperation extends RemoteOperation {
     ...
     * Constructor
     * 
     * @param file OCFile instance describing the remote file or folder to
     *            rename
    ...
    protected RemoteOperationResult run(WebdavClient client) {
        ...
        try {
            ...
            RenameRemoteFileOperation operation = new RenameRemoteFileOperation(mFile.getFileName(),
                    mFile.getRemotePath(), mNewName, mFile.isFolder());
            ...
            Log_OC.e(TAG,
                    "Rename " + mFile.getRemotePath() + " to " + ((mNewRemotePath == null) ? mNewName : mNewRemotePath)
    ...
    private void saveLocalDirectory() {
        ...
        if (localDir.exists()) {
            ...
            // TODO - if renameTo fails, children files that are already down
            // will result unlinked
        }
    }
    ...
    private void saveLocalFile() {
        ...
        if (mFile.isDown()) {
            ...
            // else - NOTHING: the link to the local file is kept although the
            // local name can't be updated
     ...
     * made in the temporal directory for downloads, out of any account, and
     * then removed.
     * 
     * IMPORTANT: The test must be made in the same file system where files are
     * download. The internal storage could be formatted with a different file
     * system.
     * 
     * TODO move this method, and maybe FileDownload.get***Path(), to a class
     * with utilities specific for the interactions with the file system
     * 
    ...
    private boolean isValidNewName() throws IOException {
        ...
        try {
            testFile.createNewFile(); // return value is ignored; it could be
                                      // 'false' because the file already
                                      // existed, that doesn't invalidate the
                                      // name
        } catch (IOException e) {
        ...
        // cleaning ; result is ignored, since there is not much we could do in
        // case of failure, but repeat and repeat...
        testFile.delete();
