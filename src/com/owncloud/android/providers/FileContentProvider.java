...
public class FileContentProvider extends ContentProvider {
    ...
    static {
        ...
        mProjectionMap.put(ProviderTableMeta.FILE_PARENT, ProviderTableMeta.FILE_PARENT);
        mProjectionMap.put(ProviderTableMeta.FILE_PATH, ProviderTableMeta.FILE_PATH);
        mProjectionMap.put(ProviderTableMeta.FILE_NAME, ProviderTableMeta.FILE_NAME);
        mProjectionMap.put(ProviderTableMeta.FILE_CREATION, ProviderTableMeta.FILE_CREATION);
        mProjectionMap.put(ProviderTableMeta.FILE_MODIFIED, ProviderTableMeta.FILE_MODIFIED);
        mProjectionMap.put(ProviderTableMeta.FILE_MODIFIED_AT_LAST_SYNC_FOR_DATA,
                ProviderTableMeta.FILE_MODIFIED_AT_LAST_SYNC_FOR_DATA);
        mProjectionMap.put(ProviderTableMeta.FILE_CONTENT_LENGTH, ProviderTableMeta.FILE_CONTENT_LENGTH);
        mProjectionMap.put(ProviderTableMeta.FILE_CONTENT_TYPE, ProviderTableMeta.FILE_CONTENT_TYPE);
        mProjectionMap.put(ProviderTableMeta.FILE_STORAGE_PATH, ProviderTableMeta.FILE_STORAGE_PATH);
        mProjectionMap.put(ProviderTableMeta.FILE_LAST_SYNC_DATE, ProviderTableMeta.FILE_LAST_SYNC_DATE);
        mProjectionMap.put(ProviderTableMeta.FILE_LAST_SYNC_DATE_FOR_DATA,
                ProviderTableMeta.FILE_LAST_SYNC_DATE_FOR_DATA);
        mProjectionMap.put(ProviderTableMeta.FILE_KEEP_IN_SYNC, ProviderTableMeta.FILE_KEEP_IN_SYNC);
        mProjectionMap.put(ProviderTableMeta.FILE_ACCOUNT_OWNER, ProviderTableMeta.FILE_ACCOUNT_OWNER);
        mProjectionMap.put(ProviderTableMeta.FILE_ETAG, ProviderTableMeta.FILE_ETAG);
    }
    ...
    private int delete(SQLiteDatabase db, Uri uri, String where, String[] whereArgs) {
        ...
        case SINGLE_FILE:
            /*
             * Cursor c = query(db, uri, null, where, whereArgs, null); String
             * remotePath = "(unexisting)"; if (c != null && c.moveToFirst()) {
             * remotePath =
             * c.getString(c.getColumnIndex(ProviderTableMeta.FILE_PATH)); }
             * Log_OC.d(TAG, "Removing FILE " + remotePath);
             */
            count = db.delete(ProviderTableMeta.DB_NAME, ProviderTableMeta._ID + "=" + uri.getPathSegments().get(1)
                    + (!TextUtils.isEmpty(where) ? " AND (" + where + ")" : ""), whereArgs);
            /*
             * just for log if (c!=null) { c.close(); }
             */
            break;
        case DIRECTORY:
            ...
            /*
             * Uri folderUri =
             * ContentUris.withAppendedId(ProviderTableMeta.CONTENT_URI_FILE,
             * Long.parseLong(uri.getPathSegments().get(1))); Cursor folder =
             * query(db, folderUri, null, null, null, null); String folderName =
             * "(unknown)"; if (folder != null && folder.moveToFirst()) {
             * folderName =
             * folder.getString(folder.getColumnIndex(ProviderTableMeta
             * .FILE_PATH)); }
             */
            ...
            if (children != null && children.moveToFirst()) {
                ...
                while (!children.isAfterLast()) {
                    ...
                    isDir = "DIR".equals(children.getString(children
                            .getColumnIndex(ProviderTableMeta.FILE_CONTENT_TYPE)));
                    // remotePath =
                    // children.getString(children.getColumnIndex(ProviderTableMeta.FILE_PATH));
                    if (isDir) {
               ...
               * else { Log_OC.d(TAG, "No child to remove in DIRECTORY " +
               * folderName); } Log_OC.d(TAG, "Removing DIRECTORY " + folderName
               * + " (or maybe not) ");
               */
            count += db.delete(ProviderTableMeta.DB_NAME, ProviderTableMeta._ID + "=" + uri.getPathSegments().get(1)
                    + (!TextUtils.isEmpty(where) ? " AND (" + where + ")" : ""), whereArgs);
            /*
             * Just for log if (folder != null) { folder.close(); }
             */
            break;
        case ROOT_DIRECTORY:
    ...
    public Uri insert(Uri uri, ContentValues values) {
        // Log_OC.d(TAG, "Inserting " +
        // values.getAsString(ProviderTableMeta.FILE_PATH) + " at provider " +
        // this);
        Uri newUri = null;
        SQLiteDatabase db = mDbHelper.getWritableDatabase();
        db.beginTransaction();
    ...
    private Uri insert(SQLiteDatabase db, Uri uri, ContentValues values) {
        ...
        String[] projection = new String[] { ProviderTableMeta._ID, ProviderTableMeta.FILE_PATH,
                ProviderTableMeta.FILE_ACCOUNT_OWNER };
        String where = ProviderTableMeta.FILE_PATH + "=? AND " + ProviderTableMeta.FILE_ACCOUNT_OWNER + "=?";
        String[] whereArgs = new String[] { remotePath, accountName };
        Cursor doubleCheck = query(db, uri, projection, where, whereArgs, null);
        if (doubleCheck == null || !doubleCheck.moveToFirst()) { // ugly patch;
                                                                 // serious
                                                                 // refactorization
                                                                 // is needed to
                                                                 // reduce work
            ...
            if (rowId > 0) {
                ...
                // Log_OC.d(TAG, "Inserted " +
                // values.getAsString(ProviderTableMeta.FILE_PATH) +
                // " at provider " + this);
                return insertedFileUri;
            } else {
                // Log_OC.d(TAG, "Error while inserting " +
                // values.getAsString(ProviderTableMeta.FILE_PATH) +
                // " at provider " + this);
                throw new SQLException("ERROR " + uri);
            }
        } else {
            // file is already inserted; race condition, let's avoid a
            // duplicated entry
            Uri insertedFileUri = ContentUris.withAppendedId(ProviderTableMeta.CONTENT_URI_FILE,
    ...
    private Cursor query(SQLiteDatabase db, Uri uri, String[] projection, String selection, String[] selectionArgs,
            String sortOrder) {
    ...
    private int updateFolderSize(SQLiteDatabase db, String folderId) {
        ...
        String[] childrenProjection = new String[] { ProviderTableMeta.FILE_CONTENT_LENGTH,
        ...
        if (childrenCursor != null && childrenCursor.moveToFirst()) {
            while (!childrenCursor.isAfterLast()) {
                childrenSize += childrenCursor.getLong(childrenCursor
    ...
    class DataBaseHelper extends SQLiteOpenHelper {
        ...
        public void onCreate(SQLiteDatabase db) {
            ...
            db.execSQL("CREATE TABLE " + ProviderTableMeta.DB_NAME + "(" + ProviderTableMeta._ID
                    + " INTEGER PRIMARY KEY, " + ProviderTableMeta.FILE_NAME + " TEXT, " + ProviderTableMeta.FILE_PATH
                    + " TEXT, " + ProviderTableMeta.FILE_PARENT + " INTEGER, " + ProviderTableMeta.FILE_CREATION
                    + " INTEGER, " + ProviderTableMeta.FILE_MODIFIED + " INTEGER, "
                    + ProviderTableMeta.FILE_CONTENT_TYPE + " TEXT, " + ProviderTableMeta.FILE_CONTENT_LENGTH
                    + " INTEGER, " + ProviderTableMeta.FILE_STORAGE_PATH + " TEXT, "
                    + ProviderTableMeta.FILE_ACCOUNT_OWNER + " TEXT, " + ProviderTableMeta.FILE_LAST_SYNC_DATE
                    + " INTEGER, " + ProviderTableMeta.FILE_KEEP_IN_SYNC + " INTEGER, "
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
            ...
            if (oldVersion == 1 && newVersion >= 2) {
                ...
                db.execSQL("ALTER TABLE " + ProviderTableMeta.DB_NAME + " ADD COLUMN "
                        + ProviderTableMeta.FILE_KEEP_IN_SYNC + " INTEGER " + " DEFAULT 0");
                upgraded = true;
            }
            if (oldVersion < 3 && newVersion >= 3) {
                ...
                try {
                    db.execSQL("ALTER TABLE " + ProviderTableMeta.DB_NAME + " ADD COLUMN "
                            + ProviderTableMeta.FILE_LAST_SYNC_DATE_FOR_DATA + " INTEGER " + " DEFAULT 0");

                    // assume there are not local changes pending to upload
                    db.execSQL("UPDATE " + ProviderTableMeta.DB_NAME + " SET "
                            + ProviderTableMeta.FILE_LAST_SYNC_DATE_FOR_DATA + " = " + System.currentTimeMillis()
            ...
            if (oldVersion < 4 && newVersion >= 4) {
                ...
                try {
                    db.execSQL("ALTER TABLE " + ProviderTableMeta.DB_NAME + " ADD COLUMN "
                            + ProviderTableMeta.FILE_MODIFIED_AT_LAST_SYNC_FOR_DATA + " INTEGER " + " DEFAULT 0");

                    db.execSQL("UPDATE " + ProviderTableMeta.DB_NAME + " SET "
                            + ProviderTableMeta.FILE_MODIFIED_AT_LAST_SYNC_FOR_DATA + " = "
                            + ProviderTableMeta.FILE_MODIFIED + " WHERE " + ProviderTableMeta.FILE_STORAGE_PATH
                            + " IS NOT NULL");
            ...
            if (!upgraded)
                Log_OC.i("SQL", "OUT of the ADD in onUpgrade; oldVersion == " + oldVersion + ", newVersion == "
                        + newVersion);
            ...
            if (oldVersion < 5 && newVersion >= 5) {
                ...
                try {
                    db.execSQL("ALTER TABLE " + ProviderTableMeta.DB_NAME + " ADD COLUMN "
                            + ProviderTableMeta.FILE_ETAG + " TEXT " + " DEFAULT NULL");
