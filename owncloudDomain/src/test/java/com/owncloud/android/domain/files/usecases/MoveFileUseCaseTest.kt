...
class MoveFileUseCaseTest {
    ...
    private val useCaseParams = MoveFileUseCase.Params(
        listOfFilesToMove = listOf(OC_FILE.copy(remotePath = "/video.mp4")),
        targetFile = OC_FOLDER
    )
    ...
    fun `move file - ko - empty list`() {
        val useCaseResult = useCase.execute(useCaseParams.copy(listOfFilesToMove = listOf(), targetFile = OC_FOLDER))
