...
class LoginOAuthAsyncUseCaseTest {
    ...
    fun `login oauth - ko - invalid params`() {
        ...
        verify(exactly = 0) { repository.loginOAuth(any(), any(), any(), any(), any(), any(), any()) }
    }

    @Test
    fun `login oauth - ok`() {
        every { repository.loginOAuth(any(), any(), any(), any(), any(), any(), any()) } returns OC_ACCOUNT_NAME
        ...
        verify(exactly = 1) { repository.loginOAuth(any(), any(), any(), any(), any(), any(), any()) }
    }

    @Test
    fun `login oauth - ko - another exception`() {
        every { repository.loginOAuth(any(), any(), any(), any(), any(), any(), any()) } throws Exception()
