...
public class OwnCloudClientFactory {
     ...
     *             If the authenticator failed to get the authorization token
     *             for the account.
     * @throws OperationCanceledException
     *             If the authenticator operation was cancelled while getting
     *             the authorization token for the account.
     * @throws IOException
     *             If there was some I/O error while getting the authorization
     *             token for the account.
     * @throws AccountNotFoundException
     *             If 'account' is unknown for the AccountManager
     */
    public static WebdavClient createOwnCloudClient(Account account, Context appContext)
        ...
        boolean isOauth2 = am.getUserData(account, OwnCloudAccount.Constants.KEY_SUPPORTS_OAUTH2) != null; // TODO
                                                                                                           // avoid
                                                                                                           // calling
                                                                                                           // to
                                                                                                           // getUserData
        ...
        if (isOauth2) {
            ...
            client.setBearerCredentials(accessToken); // TODO not assume that
    ...
    public static WebdavClient createOwnCloudClient(Account account, Context appContext, Activity currentActivity)
        ...
        if (isOauth2) { // TODO avoid a call to getUserData here
            AccountManagerFuture<Bundle> future = am.getAuthToken(account,
                    AccountTypeUtils.getAuthTokenTypeAccessToken(account.type), null, currentActivity, null, null);
            ...
            AccountManagerFuture<Bundle> future = am
                    .getAuthToken(account, AccountTypeUtils.getAuthTokenTypeSamlSessionCookie(account.type), null,
            ...
            String username = account.name.substring(0, account.name.lastIndexOf('@'));
            // String password = am.getPassword(account);
            // String password = am.blockingGetAuthToken(account,
            // MainApp.getAuthTokenTypePass(), false);
            AccountManagerFuture<Bundle> future = am.getAuthToken(account,
                    AccountTypeUtils.getAuthTokenTypePass(account.type), null, currentActivity, null, null);
            Bundle result = future.getResult();
    ...
    /**
     * Creates a WebdavClient to access a URL and sets the desired parameters
     * for ownCloud client connections.
     * 
     * @param uri
     *            URL to the ownCloud server
     * @param context
     *            Android context where the WebdavClient is being created.
     * @return A WebdavClient object ready to be used
     */
