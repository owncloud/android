...
/**
 * Remote operation performing the download of a remote file in the ownCloud
 * server.
 * 
...
public class DownloadRemoteFileOperation extends RemoteOperation {
    ...
    protected RemoteOperationResult run(WebdavClient client) {
        ...
        // / download will be performed to a temporal file, then moved to the
        // final location
        ...
        try {
            ...
            result = new RemoteOperationResult(isSuccess(status), status, (mGet != null ? mGet.getResponseHeaders()
                    : null));
            Log.i(TAG,
                    "Download of " + mRemoteFile.getRemotePath() + " to " + getTmpPath() + ": "
    ...
    protected int downloadFile(WebdavClient client, File targetFile) throws HttpException, IOException,
        ...
        try {
            ...
            if (isSuccess(status)) {
                ...
                while ((readResult = bis.read(bytes)) != -1) {
                    ...
                    synchronized (mDataTransferListeners) {
                        ...
                        while (it.hasNext()) {
                            it.next().onTransferProgress(readResult, transferred, mRemoteFile.getLength(),
                                    targetFile.getName());
                        }
                    }
                }
            ...
            mGet.releaseConnection(); // let the connection available for other
                                      // methods
        }
        return status;
    }
    ...
    public void cancel() {
        mCancellationRequested.set(true); // atomic set; there is no need of
                                          // synchronizing it
    }
}
