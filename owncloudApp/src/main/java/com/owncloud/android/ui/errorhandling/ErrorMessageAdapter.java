...
public class ErrorMessageAdapter {
    ...
    public static String getResultMessage(final RemoteOperationResult result,
        ...
        if (result.isSuccess()) {
            if (operation instanceof UploadFileOperation) {
                return f.format(R.string.uploader_upload_succeeded_content_single,
        ...
        switch (result.getCode()) {
            ...
            case FORBIDDEN:
                ...
                if (operation instanceof MoveFileOperation) {
                    return f.forbidden(R.string.forbidden_permissions_move);
                }
                if (operation instanceof CopyFileOperation) {
                    return f.forbidden(R.string.forbidden_permissions_copy);
                }
            ...
            case FILE_NOT_FOUND:
                ...
                if (operation instanceof RenameFileOperation) {
                    return f.format(R.string.rename_server_fail_msg);
                }
                if (operation instanceof MoveFileOperation) {
                    return f.format(R.string.move_file_not_found);
                }
                ...
                if (operation instanceof CopyFileOperation) {
                    return f.format(R.string.copy_file_not_found);
                }
            ...
            case INVALID_OVERWRITE:
                if (operation instanceof MoveFileOperation) {
                    return f.format(R.string.move_file_invalid_overwrite);
                }
                if (operation instanceof CopyFileOperation) {
                    return f.format(R.string.copy_file_invalid_overwrite);
                }
            case CONFLICT:
                return f.format(R.string.move_file_error);
    ...
    private static String getCommonMessageForResult(RemoteOperation operation,
        ...
        switch (result.getCode()) {
            ...
            case TIMEOUT:
                return (result.getException() instanceof SocketTimeoutException)
    ...
    private static String getGenericErrorMessageForOperation(RemoteOperation operation,
        ...
        if (operation instanceof DownloadFileOperation) {
            return f.format(R.string.downloader_download_failed_content,
                    new File(((DownloadFileOperation) operation).getSavePath()).getName());
        }
        if (operation instanceof RemoveFileOperation) {
            return f.format(R.string.remove_fail_msg);
        }
        if (operation instanceof RenameFileOperation) {
            return f.format(R.string.rename_server_fail_msg);
        }
        if (operation instanceof CreateFolderOperation) {
            return f.format(R.string.create_dir_fail_msg);
        }
        if (operation instanceof CreateShareViaLinkOperation ||
        ...
        if (operation instanceof RemoveShareOperation) {
            return f.format(R.string.unshare_link_file_error);
        }
        ...
        if (operation instanceof CopyFileOperation) {
            return f.format(R.string.copy_file_error);
        }
        // if everything else failes
        if (result.isSuccess()) {
            return f.format(android.R.string.ok);
        } else {
            return f.format(R.string.common_error_unknown);
        }
    }
}
