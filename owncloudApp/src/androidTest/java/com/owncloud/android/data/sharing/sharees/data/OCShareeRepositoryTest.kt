...
package com.owncloud.android.data.sharing.sharees.data
...
class OCShareeRepositoryTest {
    ...
    fun loadShareesFromNetworkSuccessfully() {
        val remoteSharees: ArrayList<JSONObject> = arrayListOf(
            TestUtil.createSharee("Thor", "0", "thor", "thor@avengers.com"),
            TestUtil.createSharee("Iron Man", "0", "ironman", "ironman@avengers.com"),
            TestUtil.createSharee("Avengers", "1", "avengers", "everybody@avengers.com")
        )

        val getRemoteShareesOperationResult =
            TestUtil.createRemoteOperationResultMock(arrayListOf(remoteSharees[1]), true)
    ...
    fun loadEmptyShareesFromNetwork() {
        ...
        val getRemoteShareesOperationResult =
            TestUtil.createRemoteOperationResultMock(remoteSharees, true)
    ...
    fun loadEmptyShareesFromNetworkWithError() {
        val remoteSharees: ArrayList<JSONObject> = arrayListOf(
            TestUtil.createSharee("Joker", "0", "joker", "joker@dc.com")
        )

        val exception = Exception("Error when retrieving sharees")

        val getRemoteShareesOperationResult =
            TestUtil.createRemoteOperationResultMock(
                remoteSharees,
                false,
