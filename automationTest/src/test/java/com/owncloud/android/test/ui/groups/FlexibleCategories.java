...
public class FlexibleCategories extends Suite {
    ...
    /**
     * Specifies an annotation for methods which must be present in a matching class (e.g. @TestMethodAnnotationFilter
     * (Test.class)).
    ...
    private static String getTestScanPackage(Class<?> clazz) throws InitializationError {
        ...
        if (annotation == null) {
            throw new InitializationError("No package given to scan for tests!\nUse the annotation @TestScanPackage" +
                    "(\"my.package\") on the test suite " + clazz + ".");
        }
    ...
    private void assertNoDescendantsHaveCategoryAnnotations(Description description) throws InitializationError {
        for (Description each : description.getChildren()) {
            if (each.getAnnotation(Category.class) != null) {
                throw new InitializationError("Category annotations on Parameterized classes are not supported on " +
                        "individual methods.");
            }
