...
class SearchRemoteUsersOperation(
    ...
    override fun run(client: OwnCloudClient): RemoteOperationResult<List<MemberResponse>> {
        ...
        try {
            ...
            val uriBuilder = client.baseUri.buildUpon().apply {
                appendEncodedPath(GRAPH_API_USERS_PATH)
            ...
            if (status == HttpConstants.HTTP_OK) {
                ...
                Timber.d("Get all available users completed and parsed to ${result.data}")
            } else {
                result = RemoteOperationResult(getMethod)
                Timber.e("Failed response while getting the users; status code: $status, response: $response")
            }
        } catch (e: Exception) {
            result = RemoteOperationResult(e)
            Timber.e(e, "Exception while getting the users")
        }
        return result
    }
