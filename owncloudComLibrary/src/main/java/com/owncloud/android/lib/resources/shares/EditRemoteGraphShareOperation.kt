...
class EditRemoteGraphShareOperation(
    ...
    override fun run(client: OwnCloudClient): RemoteOperationResult<Unit> {
        ...
        try {
            ...
            if (status == HttpConstants.HTTP_OK) {
                ...
                Timber.d("Edit graph share operation completed")
            } else {
                result = RemoteOperationResult(patchMethod)
                Timber.e("Failed response while editing a graph share; status code: $status, response: $response")
            }
        } catch (e: Exception) {
            result = RemoteOperationResult(e)
            Timber.e(e, "Exception while editing a graph share")
        }
        return result
    }
