...
class AddRemoteGraphShareOperation(
    ...
    override fun run(client: OwnCloudClient): RemoteOperationResult<Unit> {
        ...
        try {
            val uriBuilder = client.baseUri.buildUpon().apply {
                ...
                appendEncodedPath(GRAPH_API_ITEMS_PATH)
                appendEncodedPath(itemId)
                appendEncodedPath(GRAPH_API_INVITE_PATH)
            }

            val recipients = JSONObject().apply {
            ...
            val requestBody = JSONObject().apply {
                ...
                put(RECIPIENTS_BODY_PARAM, JSONArray().put(recipients))
                put(ROLES_BODY_PARAM, JSONArray().put(roleId))
            ...
            when (status) {
                HttpConstants.HTTP_OK -> {
                    ...
                    Timber.d("Add graph share operation completed")
                }
                HttpConstants.HTTP_CONFLICT -> {
                    result = RemoteOperationResult(ResultCode.CONFLICT_SHARE)
                    Timber.e("Conflict response while adding a graph share; status code: $status, response: $response")
                }
                else -> {
                    result = RemoteOperationResult(postMethod)
                    Timber.e("Failed response while adding a graph share; status code: $status, response: $response")
                }
            }
        } catch (e: Exception) {
            result = RemoteOperationResult(e)
            Timber.e(e, "Exception while adding a graph share")
        }
        return result
    }
    ...
    companion object {
        ...
        private const val GRAPH_API_ITEMS_PATH = "items"
        private const val GRAPH_API_INVITE_PATH = "invite"
