...
//Working on it. It isn´t working right now.
    @Test
    fun `loginOAuth successful login`() = runBlocking {
        ...
        val expectedResult = Pair(userInfo, serverPath)

        val mock = spyk(OCRemoteAuthenticationDataSource(clientManager), recordPrivateCalls = true)

        every { mock["login"](any(), any()) } returns expectedResult
        val result = mock.loginOAuth(serverPath, username, password) // Aquí utilizamos el mock
